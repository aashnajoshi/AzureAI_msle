...
using Azure;
using Azure.AI.Language.LUIS.Runtime;
    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            try
            {
                ...
                string endpoint = $"https://{predictionRegion}.api.cognitive.microsoft.com";
                var credentials = new AzureKeyCredential(predictionKey);
                var client = new LUISRuntimeClient(new Uri(endpoint), credentials);

                // Get the model from the AppID and add the intents we want to use
                var model = await client.GetPredictionAsync(luAppId, "production", "en-us", "What's the time in New York?");

                // Process speech input
                string intent = GetIntent(model);
